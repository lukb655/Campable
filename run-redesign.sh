#!/bin/bash
# Campable Redesign — Headless Runner
# Wraps campable-redesign.sh fixes with --dangerously-skip-permissions and logging.
# Hook must be enabled — each commit auto-creates an auto/<hash> PR.
# Run: bash run-redesign.sh

LOG="redesign_$(date +%Y%m%d_%H%M%S).log"
DELAY=15
...
log "Campable Redesign Runner"
log "Project: $(pwd)"
log "Started: $(date)"
log "Log:     $LOG"

run_fix 1 "Navigation: 3-Button + Expandable Center Menu" \
"Rebuild the bottom navigation bar with exactly 3 buttons. Left button: Home, navigates to Home tab. Right button: Crew, navigates to Crew tab. Center button: styled larger and elevated with an accent color. When tapped, it expands upward into a vertical column of 5 buttons: Meals, Costs, Packing List, Waypoints, Itinerary. Tapping any of these navigates to that page and collapses the menu. Tapping center again collapses without navigating. Use smooth CSS transition for expand/collapse. Center menu renders above all content via z-index. Preserve all existing app styles, colors, and component conventions. When done, commit all changes with a descriptive message."

run_fix 2 "Home Page: Customizable Widget Tiles" \
